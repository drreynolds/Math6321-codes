/* Explicit 3rd-order Runge-Kutta time stepper class implementation file.
...
// The explicit 3rd-order Runge-Kutta time step evolution routine
//
...
//          all tspan values,
//            [y(t0), y(t1), ..., y(tN)]
mat ERK3Stepper::Evolve(vec tspan, double h, vec y) {
  ...
  for (size_t tstep=0; tstep<N; tstep++) {
    ...
    for (size_t i=0; i<Nint; i++) {
      ...
      if (frhs->Evaluate(t, y, k0) != 0) {
        std::cerr << "ERK3Stepper::Evolve: Error in ODE RHS function\n";
        return Y;
      }
